...
const saveTweet = (uid, tweetObject) => new Promise((resolve, reject) => {
  getSavedTweets(uid).then((response) => {
    const checkExistingTweet = response.filter((tweet) => tweet.tweetId === tweetObject.tweetId);
    if (checkExistingTweet.length === 0) {
      axios.post(`${dbUrl}/savedTweets.json`, tweetObject)
        .then((res) => {
...
const deleteSavedTweet = (uid, id) => new Promise((resolve, reject) => {
  axios.delete(`${dbUrl}/savedTweets/${id}.json`)
    .then(() => {
