...
const getSingleTopic = (topicId) => new Promise((resolve, reject) => {
  axios
    .get(`${dbUrl}/topics/${topicId}.json`)
    .then((response) => resolve(response.data))
    .catch((error) => reject(error));
});
const getFavoriteTopics = (uid) => new Promise((resolve, reject) => {
  axios
    .get(`${dbUrl}/topics.json?orderBy="uid"&equalTo="${uid}"`)
...
const createTopic = (topicObject) => new Promise((resolve, reject) => {
  axios
    .post(`${dbUrl}/topics.json`, topicObject)
    .then((response) => {
      const body = { id: response.data.name };
      axios
        .patch(`${dbUrl}/topics/${response.data.name}.json`, body)
...
const editTopic = (topic, topicId) => new Promise((resolve, reject) => {
  axios
    .patch(`${dbUrl}/topics/${topicId}.json`, topic)
