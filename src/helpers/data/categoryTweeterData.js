...
const getCategoryTweeterInfo = (categoryId) => new Promise((resolve, reject) => {
  getCategoryTweeters(categoryId)
    .then((response) => {
      if (response.length !== 0) {
        getTweeterInfo(response.map((tweeter) => tweeter.twitterId)).then(
          resolve
        );
      } else {
        resolve([]);
...
const createTweeter = (categoryId, tweeterObject) => new Promise((resolve, reject) => {
  axios
    .post(`${dbUrl}/tweeters.json`, tweeterObject)
    .then((response) => {
      const body = { id: response.data.name };
      axios
        .patch(`${dbUrl}/tweeters/${response.data.name}.json`, body)
        .then(() => getCategoryTweeterInfo(categoryId).then(resolve));
    })
    .catch((error) => reject(error));
});
const getSingleTweeterbyUsername = (username) => new Promise((resolve, reject) => {
  axios
    .get(`${dbUrl}/tweeters.json?orderBy="twitterId"&equalTo="${username}"`)
    .then((response) => resolve(Object.values(response.data)[0]))
    .catch((error) => reject(error));
});
const deleteTweeter = (username, categoryId) => new Promise((resolve, reject) => {
  getSingleTweeterbyUsername(username.toLowerCase())
    .then((response) => {
      axios.delete(`${dbUrl}/tweeters/${response.id}.json`).then(() => {
...
const deleteCategory = (uid, categoryId) => new Promise((resolve, reject) => {
  axios
    .delete(`${dbUrl}/categories/${categoryId}.json`)
    .then(() => {
...
export {
  createTweeter,
  getCategoryTweeterInfo,
  deleteTweeter,
  deleteCategory,
  getSingleTweeterbyUsername,
};
