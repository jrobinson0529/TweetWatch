...
const createUser = (userObject) => new Promise((resolve, reject) => {
  axios
    .post(`${dbUrl}/users.json`, userObject)
    .then((response) => {
      const body = { id: response.data.name };
      axios
        .patch(`${dbUrl}/users/${response.data.name}.json`, body)
        .then((res) => getUserInfo(res.data.id).then(resolve));
    })
    .catch((error) => reject(error));
});
const editUser = (userObject, id) => new Promise((resolve, reject) => {
  axios
    .patch(`${dbUrl}/users/${id}.json`, userObject)
    .then((response) => resolve(response.data))
    .catch((error) => reject(error));
});
