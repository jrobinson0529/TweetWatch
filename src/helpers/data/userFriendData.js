...
const getSingleUserFriend = (uid, friendId) => new Promise((resolve, reject) => {
  axios
    .get(`${dbUrl}/friendedUsers.json?orderBy="uid"&equalTo="${uid}"`)
    .then((response) => {
      resolve(
...
const mergeUserFriendInfo = (friendArray) => new Promise((resolve, reject) => {
  ...
  Promise.all(friendInfoPromiseArray)
    .then((response) => {
      resolve(response);
    })
    .catch((error) => reject(error));
...
const createUserFriend = (uid, friendId) => new Promise((resolve, reject) => {
  getSingleUserFriend(uid, friendId)
    .then((friendResponse) => {
      if (friendResponse.length <= 0) {
        ...
        axios.post(`${dbUrl}/friendedUsers.json`, object).then((response) => {
          ...
          axios.patch(
            `${dbUrl}/friendedUsers/${response.data.name}.json`,
            body
          );
        });
      }
  ...
  axios
    ...
    .then((response) => {
      if (Object.values(response.data)) {
        createUserFriend(uid, Object.values(response.data)[0].uid).then(
          resolve
        );
      }
    })
...
const deleteUserFriend = (uid, id) => new Promise((resolve, reject) => {
  getUserFriends(uid)
    .then((x) => mergeUserFriendInfo(x).then((response) => {
      const singleFriend = response.filter((friend) => friend.id === id);
      getSingleUserFriend(uid, singleFriend[0].uid).then(
        (friendResponse) => {
          axios
            .delete(`${dbUrl}/friendedUsers/${friendResponse[0].id}.json`)
